...
int main(int argc, char **argv)
{
    ...
    bool use_system_prompt = false;
    if (argc >= 5) {
        string sys = read_file(argv[4]);
        if (!sys.empty()) {
            ret = rkllm_set_chat_template(llmHandle,
                sys.c_str(),
                "<|im_start|>user\n",
                "<|im_end|>\n<|im_start|>assistant");
            if (ret == 0) {
                printf("system prompt loaded (%zu bytes)\n", sys.size());
                use_system_prompt = true;
            } else {
                fprintf(stderr, "rkllm_set_chat_template failed: %d\n", ret);
            }
        }
    }
    ...
    rkllm_infer_params.keep_history = use_system_prompt ? 1 : 0;
